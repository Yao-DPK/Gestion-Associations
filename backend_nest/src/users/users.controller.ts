...
export class UsersController {
  ...
  async create(@Body() input: UserInput) {
    try {
      ...
      return await this.service.create(input.firstname, input.lastname, input.age, hashedPassword);
    } catch (error) {
      if (error instanceof HttpException) {
        throw error;
      } else {
  ...
  async updatebyId(@Param('id') id: string, @Body() input: UserInput) {
    try {
      ...
      return await this.service.updatebyId(+id, input.firstname, input.lastname, input.age, hashedPassword);
    } catch (error) {
      if (error instanceof HttpException) {
        throw error;
      } else {
